...
Schema.prototype.writeController = function(name) {
  name = ucwords(name, false)+'Controller.js';
...
function generateModel(schema) {
  ...
  schema.forEach(definition => {
    ...
    identity = ucwords(definition.table_name, 0).toLowerCase();

    if(definition.table_type === 'VIEW'){
        isView = true;
        modelAttributes['id'] = { type: 'number', required: true };
    }
    modelAttributes[ucwords(definition.column_name, 1)] = generateColumn(definition);

    if (definition.column_key === 'PRI') {
        primaryKey = ucwords(definition.column_name, 1);
    }
    ...
    if (definition.referenced_table_name) {
      ...
      relationships.forEach(rel => {
          if(rel.hasOwnProperty(ucwords(definition.referenced_table_name, 0).toLowerCase())){
              addRel = false;
          }
      });
      ...
      if(addRel){
          ...
          relationship[ucwords(definition.referenced_table_name, 0).toLowerCase()] = { model: {} };
          relationship[ucwords(definition.referenced_table_name, 0).toLowerCase()].model = ucwords(definition.referenced_table_name, 0).toLowerCase();
...
function ucwords(name, isAttr) {
  if (isAttr) {
    return name.replace(/_/g, ' ').replace(/(?:^\w|[A-Z]|\b\w)/g, function(letter, index) {
        return index === 0 ? letter.toLowerCase() : letter.toUpperCase();
    }).replace(/\s+/g, '');
  } else {
    return name[0].toUpperCase() + name.substr(1).replace(/_/g, ' ').replace(/(?:^\w|[A-Z]|\b\w)/g, function(letter, index) {
        return index === 0 ? letter.toLowerCase() : letter.toUpperCase();
    }).replace(/\s+/g, '');
  }
